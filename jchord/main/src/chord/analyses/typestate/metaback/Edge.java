...
public class Edge implements IEdge {
    ...
    public int canMerge(IEdge e, boolean mustMerge) {
        ...
        Edge that = (Edge) e;
        if (this.type != that.type || this.h != that.h) 
        	return -1;
        if(!Utils.areEqual(this.srcNode, that.srcNode) )
        	return -1;
        if (this.dstNode != null && that.dstNode != null) {
            if (this.dstNode.canReturn != that.dstNode.canReturn) 
            	return -1;
            if(this.dstNode.isError != that.dstNode.isError)
            	return -1;
            ArraySet<Register> thisMS = this.dstNode.ms;
            ArraySet<Register> thatMS = that.dstNode.ms;
            if(thisMS.equals(thatMS))
            	return 0;
