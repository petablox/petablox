...
<xsl:template match="/results/resultlist">
	...
	<head>
		...
		<table border="1" class="details">
				...
				<tr>
					<td class="head2">
					Variables needed to track ( 
					number=
					<xsl:value-of select="TVS/@size"/> )
					</td>
				</tr>
					<xsl:for-each select="TVS/V">
				<tr>
					<td class="var">
					 <xsl:apply-templates select="id(@Vid)"/> (
					 <xsl:value-of select="."/>
					 )
					</td>
				</tr>
					...
					<tr>
					<td class="proven">
					<xsl:apply-templates select="id(@Iid)"/>
					#########
