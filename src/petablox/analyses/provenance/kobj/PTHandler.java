...
public class PTHandler implements ParamTupleConsHandler {
	...
	public Set<FormatedConstraint> getHardCons(int w, Set<Tuple> paramTSet, MaxSatGenerator gen) {
		...
		Map<Unit,List<Tuple>> qOMap = new HashMap<Unit,List<Tuple>>();
		Map<Unit,List<Tuple>> qHMap = new HashMap<Unit,List<Tuple>>();
		for(Tuple t : paramTSet){
			String relName = t.getRelName();
			Map<Unit,List<Tuple>> qToTMap;
			if(relName.equals("DenyH"))
				qToTMap = qHMap;
			else
				qToTMap = qOMap;
		...
		for(Map.Entry<Unit, List<Tuple>> entry: qOMap.entrySet()){
		...
		for(Map.Entry<Unit, List<Tuple>> entry: qHMap.entrySet()){
			List<Tuple> l = entry.getValue();
			for(int i = 0; i < l.size() - 1; i++){
				Tuple t1 = l.get(i);
				Tuple t2 = l.get(i+1);
