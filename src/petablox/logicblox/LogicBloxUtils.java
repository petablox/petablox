...
public class LogicBloxUtils {
    ...
    public static void setMultiTag() {
    	if (Config.multiPgmMode) {
	    	if (Config.multiTag.equals("")) {
				Config.multiTag = Utils.getAutoGeneratedMultiTag();
				Messages.log("MULTIPGM: Auto-generated tag for relation names: " + Config.multiTag);
	    	}
	    	Config.multiTag = Config.multiTag + "_";
			if (tagContains(Config.multiTag)) 
				Messages.fatal("MULTIPGM: The tag specified by petablox.multipgm.tag is already present.");
			if (!Config.analyze.equals("")) {
				String[] parts = Config.analyze.split(",");
				StringBuilder sb = new StringBuilder();
				for(String s : parts) sb.append(s + "_" + ",");
				sb.setLength(sb.length() - 1); // Remove the last comma
				Config.analyze = sb.toString();
			}
    	}
    }
    ...
    public static void validateMultiPgmOptions() {
        ...
        if (Config.populate && !Config.analyze.equals(""))
