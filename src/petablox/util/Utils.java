...
public final class Utils {
    ...
    private static String autoGenMultiTag = "";
...
//            .replace("'", "&apos;").replaceAll("\"", "&quot;");
    ...
    public static String getAutoGeneratedMultiTag(ArraySet<String> oldTags) {
    	if (autoGenMultiTag.equals("")) {
	    	String workDir;
	    	//String tagPrefix = "T";
	    	...
	    	int ndx = 0;
	    	File ndxFile = new File(workDir, "autoMultiTagNdx.txt");
	    	if (ndxFile.exists()) {
				try {
					Scanner sc = new Scanner(ndxFile);				
					String line = sc.nextLine().trim();
					ndx = Integer.parseInt(line);
					sc.close();
	    	...
	    	String tag = convertToStr(ndx);
	    	String temp = tag + "tag";
	    	boolean unique = false;
	    	while (!unique) {
	    		if (oldTags.contains(temp)) {
	    			ndx++;
	    			tag = convertToStr(ndx);
	    			temp = tag + "tag";
	    		} else
	    			unique = true;
	    	}
