...
import petablox.analyses.lock.DomR;
...
public class Instrumentor extends BasicInstrumentor {
    ...
    protected final SootMethod mainMethod;
    ...
    protected SootClass currClass;
    protected SootMethod currMethod;
    ...
    protected int getBCI(Block b, SootMethod m) {
        Iterator<Unit> uit=b.iterator();
        while (uit.hasNext()) {
            Unit q = uit.next();
            int bci = SootUtilities.getBCI(q);
            if (bci != -1)
                return bci;
        }
    ...
    public CtClass edit(CtClass clazz) throws CannotCompileException {
        ...
        RefLikeType t = program.getClass(cName);
        if(t instanceof ArrayType)
        	return null;
        RefType t1 = (RefType)t;
        currClass =  t1.getSootClass();
        if (currClass == null) {
    ...
    public void edit(CtBehavior method) throws CannotCompileException {
        ...
        if (genQuadEvent || genBasicBlockEvent) {
            Map<Unit, Integer> bcMap;
            try{
                bcMap = SootUtilities.getBCMap(currMethod);
                    ...
                    if (genQuadEvent) {
                        ...
                        while(uit.hasNext()){
                            ...
                            if (isRelevant(q)) {
                                ...
                                if (pId == -1)
                                    throw new CannotCompileException("Cannot find index of Unit " + q + " in domain P");
                                int bci = SootUtilities.getBCI(q);
                                if (bci == -1)
                                    throw new CannotCompileException("Cannot find index of Unit " + q + " in bytecode");
