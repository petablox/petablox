...
import stamp.analyses.ondemand.TaintAnalysis;
...
public class IccgOnDemand extends TaintAnalysis
{
						   ...
						   Object data)
	{
		...
		if(callee.getSubSignature().equals("void onClick(android.view.View)")){
			Local viewParam = callee.retrieveActiveBody().getParameterLocal(0);
			for(AllocAndContext obj : (AllocAndContextSet) dpta.pointsToSetFor(viewParam, calleeContext)){
				Set<String> taint = taintManager.getTaint(obj.alloc);
				if(taint != null){
					if(ws == null)
						ws = new HashSet();
					ws.addAll(taint);
