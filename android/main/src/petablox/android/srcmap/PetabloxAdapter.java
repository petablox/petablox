 ...
 * @author Saswat Anand
 */
public class PetabloxAdapter extends ASTVisitor
{
	...
	public PetabloxAdapter(CompilationUnit cu, File outputFile)
	{
		try{
		...
		for(Iterator it = args.iterator(); it.hasNext();){
			...
			if(arg != null){
				...
				if(argType.isAnonymous()){
					writer.println("\t\t\t<expr"+
								   " type=\""+PetabloxSigFactory.getSyntheticName(argType)+"\""+
								   "><![CDATA["+arg+"]]></expr>");
				}
				else
	...
	private void writeSimpleMarker(String markerType, String chordSig, int lineNum, Expression expr)
	{
		...
		if(exprType.isAnonymous()){
			writer.println("\t\t\t<expr"+
						   " type=\""+PetabloxSigFactory.getSyntheticName(exprType)+"\""+
						   "><![CDATA["+expr+"]]></expr>");
		} else {
			writer.println("\t\t\t<expr><![CDATA["+expr+"]]></expr>");
		}
	...
	public static String chordSigFor(IVariableBinding v)
	{
		...
		return v.getName()+":"+toPetabloxType(v.getType())+"@"+chordClassName(declKlass.getErasure());
	}
