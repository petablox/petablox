\xname{project}
\chapter{A Petablox Project: Tasks, Targets, and Dependencies}
\label{chap:project}
...
depending upon whether the value of property \code{petablox.classic} is true or false, respectively.
\begin{itemize}
\item
a {\it classic project}, represented as an object of
class \javadoc{petablox.project.ClassicProject}{petablox/project/ClassicProject.html}.
\item
a {\it modern project}, represented as an object of
class \javadoc{petablox.project.ModernProject}{petablox/project/ModernProject.html}.
\end{itemize}
...
\hline
 Form: &
	an instance of the \code{@Petablox}-annotated class &
	\begin{tabular}{c}
...
Each task in Petablox is of the form ``\code{\{ C1, ..., Cn \} T \{ P1, ..., Pm \}}" where:
\begin{itemize}
\item
...
\code{petablox.run.analyses} as a comma-separated list of the names of tasks to be run in order
(see Chapter \ref{chap:running}).

We next illustrate the above concepts using the above example.
Suppose Petablox is run with the value of property \code{petablox.run.analyses} as ``{\tt T4}''.
...
specify the value of property \code{petablox.run.analyses} as ``{\tt T1},{\tt T4}''.  This
